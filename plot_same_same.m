...
function [ avsame, avsame2, avsame3, avdiff, avdiff2, avdiff3,sigA, avsame_group, avdiff_group, avsame_same, avdiff_same, avsame_diff, avdiff_diff,...
    trlwsesame, trlwsediff, trlwsesame2, trlwsediff2, trlwsesame3, trlwsediff3, trlnumsame, trlnumdiff] = plot_same_same(freq,firsts)

% function plot_same_same
%
...
if freq==1
    files = dir('D:\Data\DBS-MEG\Beta_Summary\LN*allchans_avgfreq_PB_cue.mat');%all channels with beat a-priori
elseif freq==2
    files = dir('D:\Data\DBS-MEG\LFO_Summary\LN*allchans_cue.mat');%all channels
end

n = numel(files);
for i = 1:n
    if freq==1 
        load (['Beta_Summary\' files(i).name]);
    elseif freq==2
        load (['LFO_Summary\' files(i).name]);
    end
    ...
    for t = 1:length(seq)
        ...
        weirdos=[8,11];%missing STN contacts
    
        if i==weirdos(1) %no R01 channel
        ...
        elseif i==weirdos(2) %no left channels
        ...
        for c=1:6 %cycle through each contact and average
    ...
    chans={'STN_R01';'STN_R12';'STN_R23';'STN_L01';'STN_L12';'STN_L23'};
    ...
    for c=1:6
        ...
        ylim([-0.1 0.1])
                ...
                cfg.clusterthreshold = 'parametric'; %use para for finding the clusters
                cfg.clusteralpha  = 0.05;%cluster forming threshold...it's complicated and FSL uses the same, that first you create some clusters, which need to be at a certain threshold, then you do stats on those, which is the values below
                cfg.alpha       = 0.05;
    ...
    if freq==1;suptitle ('Beta power');elseif freq==2; suptitle ('LFO power');elseif freq==3; suptitle ('ERP');elseif freq==4; suptitle ('Gamma');end
    
                %make data struc for FT
    ...
    if freq==1;ylabel ('Beta power ACROSS ALL CONTACTS', 'FontSize',15);elseif freq==2; ylabel ('LFO  power ACROSS ALL CONTACTS', 'FontSize',15);end
...
%% plot difference wave with errors bars
...
ylabel ('BETA Diff-Same (+sem)', 'FontSize',15);
xlim([-0.5 2.4])
...
%% detail at cue i+1
    ...
    xlabel ('Time from cue onset', 'FontSize',15);
    if freq==1;ylabel ('Beta power', 'FontSize',15);elseif freq==2; ylabel ('LFO power', 'FontSize',15);elseif freq==3; ylabel ('ERF', 'FontSize',15);elseif freq==4; ylabel ('Gamma', 'FontSize',15);end
    set(gca,'FontSize',15)
