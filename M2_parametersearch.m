...
function [LAMBDA,  LogL2, trialnum] = M2_parametersearch(P)
...
if P==1
    ...
    for s=1:size(behav,2)
        ...
        params = fminsearch (@negcor, 0, [], combosession);
        ...
        for n=1:size(combosession.stimuli,2)
            ...
            for ss=1:size(current_left,2)
                ...
                DV2(n)= (1-LAMBDA(s))*DV2(n) + current_left(ss)*-1;%M2
                        
            end
        end
        
       %estimate logL within subj along with the lambda and weights
        Y2=pdf('Logistic',DV2,choice); LogL2(s) = -sum(log(Y2));
...
function [R2]= negcor (params, combosession)

    lambda = params(1);
    
    if lambda > 1 
        R2 = 1000000000;%make sure fminsearch is bounded to certain values
    else
                ...
                for n=1:size(combosession.stimuli,2)
                    ...
                    for ss=1:size(current_left,2)  
                           ...
                           DV2(n)=DV2(n) + current_left(ss)*-1 - lambda*DV2(n);        
           ...
           rm=find(choice==3);choice(rm)=[];DV2(rm)=[];
                                       
         temp = -corrcoef(cat(2,DV2',choice'));R2 = temp(1,2);   
