...
%%
cd D:\Data\SOURCE\extracted_vertex_timecourses
...
for s = 1:13
    ...
    eval(['load(''',subjects{s},'_raw_timecourse_BF.mat'')']);
...
TF=1;
granger=0;
%find some channel combos and take only that data for similicity
for s=1:13
...
%% stats
cfg=[];
cfg.statistic = 'ft_statfun_depsamplesT';%'ft_statfun_diff';
...
for s=1:n
    ...
    if granger ==0
        ...
        S(s)=nanmean(nanmean(squeeze(group_cohS_red{s}.cohspctrm(sigchIND,sigfreqCOH,sigtimeIND_adj))));
        D(s)=nanmean(nanmean(squeeze(group_cohD_red{s}.cohspctrm(sigchIND,sigfreqCOH,sigtimeIND_adj))));
